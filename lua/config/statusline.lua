...
-- Палитра берётся заново на каждый вызов, а не один раз при загрузке: paint()
-- висит на ColorScheme, и get_palette() отдаёт цвета того flavour, который
-- активен в момент вызова — даже если тему переключили руками из командной
-- строки.
--
