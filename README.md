...
- `<CR>` или `l` — войти в директорию / открыть файл
- `h` — выйти на уровень выше
- `q` или `<Esc>` — закрыть
- `g?` — показать **все** биндинги (внутри окна explorer'а)
- `gh` — toggle скрытых файлов
- `<C-w>v` / `<C-w>s` — открыть в vsplit / hsplit
- `<C-w>t` — открыть в новой табе

**Создание / удаление / переименование** делается редактированием буфера как обычного текста, потом `:w` синхронизирует с диском. Например:
- Удалить строку → файл удалится при `:w`
- Изменить имя в строке → файл переименуется
- Добавить новую строку с именем → файл создастся (или `/` в конце — директория)
